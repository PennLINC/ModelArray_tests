...
```{r Graham Baum's code for GAM plotting}
...
My understanding of Bart Larsen's code for plotting GAM: in the case of smooth term only (without interaction term):
...
```{r Bart's code for plotting GAM}
...
load(fn.rdata)  # variable: results
...
Because this formula only includes smooth term, but there is no interaction term, for Bart's code, we will only use the chunk on and after line 281 "No interaction variable, just produce a single line plot".
